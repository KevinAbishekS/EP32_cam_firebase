...
const char* apssid = "Kevin1";
const char* appassword = "PASSWORD";         //AP密碼至少要8個字元以上  
String GOOGLE_SCRIPT_ID = "AKfycbysRmB16kwIipXApFqCEABgVozEALz8e5r7998cwioBsx2SzjmSS3vp2y2pNcsmwD48Vg"; 
//人臉辨識同一人人臉註冊影像數
...
#define FIREBASE_HOST "https://esp32-e8fca-default-rtdb.firebaseio.com/"
#define FIREBASE_AUTH "rIsR4c3PWHe9FlrDyQdLEFQTTijvlDbRKJuQ5bdN"
...
void FaceMatched(int faceid) {  //辨識到註冊人臉執行指令控制
  ...
  if (faceid==0) {
    String name(recognize_face_matched_name[0]);  //firebase 
//    sendData("tag=adc_A0&value="+name);         //firebase 
  Serial.println(name);                           //firebase 
  json.set("/name", name);                        //firebase 
  Firebase.updateNode(firebaseData,"/Sensor",json);  //firebase 
  delay(5000);
  } 
  else if (faceid==1) {   
  } 
